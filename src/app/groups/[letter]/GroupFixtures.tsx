    ...
    >
      ...
      {m.status.state === 'in' ? (
        <span className="s-status s-live">
          {m.status.isShootout ? <><span className="pulse-dot" />PENALTIES</> : m.status.isHalftime ? <span style={{ color: 'var(--live-ink)', fontWeight: 700 }}>HALF TIME</span> : <><span className="pulse-dot" />{m.status.clock}</>}
        </span>
      ) : m.status.state === 'post' ? (
